...
use crate::{
    ...
    query::{AccessKeyHandler, QueryBuilder, RpcBuilder, SimpleQuery},
};
...
impl Signer {
    ...
    pub async fn fetch_tx_nonce(
        ...
        let querier: QueryBuilder<AccessKeyHandler> = RpcBuilder::new(
            SimpleQuery { request },
            Reference::Optimistic,
            Default::default(),
        );
        let nonce_data = querier.fetch_from(network).await?;
        ...
        debug!(target: SIGNER_TARGET, "Nonce fetched and cached");
